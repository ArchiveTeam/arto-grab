...
LUA_SHA1 = get_hash(os.path.join(CWD, 'arto.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            "--warc-header", "arto-dld-script-version: " + VERSION,
            "--warc-header", ItemInterpolation("arto-user: %(item_name)s"),
        ]
        ...
        assert item_type in ('10profiles')

        if item_type == '10profiles':
            for suffix in string.digits:
                wget_args.append('http://arto.com/section/user/profile/?id={0}{1}'.format(item_value, suffix))
        else:
